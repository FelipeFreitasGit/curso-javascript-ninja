(function(){

  /*
  Envolva todo o código desse desafio em uma IIFE.
  Crie um arquivo chamado index.html e adicione esse script ao HTML.
  */

  /*
  Crie uma função construtora chamada "Person". Esse construtor deve ter
  as seguintes características:
  - Deve receber 3 parâmetros: `name`, `lastName` e `age`;
  - Deverá ter 3 propriedades:
    - `name` - que receberá o valor do parâmetro `name`;
    - `lastName` - que receberá o valor do parâmetro `lastName`;
    - `age` - que receberá o valor do parâmetro `age`;
  - Deverá ter 3 métodos:
    - `getFullName` - que deve retornar o nome completo do objeto criado,
    no formato:
      - "[NAME] [LASTNAME]"
    - `getAge` - que deverá retornar a idade (age);
    - `addAge` - esse método não deverá ter nenhum parâmetro, mas ao invocá-lo
    iremos passar um único argumento, que é a quantidade de anos que devem ser
    adicionados à idade original (age). Esse método deverá retornar o objeto
    que será instanciado.
  */
  function Person(name, lastName, age){
    this.nome = name;
    this.lastNome = lastName;
    this.idade = age;

    this.getFullName = function FullName(){
      return this.nome + ' ' + this.lastNome;
    };
    this.getAge = function Age(){
      return this.idade;
    };
    this.addAge = function aaddAge(){
      this.idade += arguments[0];

      return this;
    };
  }

  /*
  Crie 3 novos objetos usando o construtor acima. Os objetos serão novas
  pessoas. As variáveis deverão ser o primeiro nome da pessoa. Passe os
  parâmetros corretamente para o construtor para criar as novas pessoas.
  Mostre as 3 novas pessoas criadas no console (Um console.log por pessoa).
  */
  console.log( 'Novas pessoas criadas à partir de Person:' );

  var felipe = new Person('felipe','vasconcellos',22);
  var fernando = new Person('fernando','vasconcellos',22);
  var kelly =  new Person('kelly','haro',23);
  console.log(felipe);
  console.log(fernando);
  console.log(kelly);

  /*
  Mostre no console o nome completo de cada pessoa.
  */
  console.log( '\nNomes das pessoas:' );
  console.log(felipe.getFullName());
  console.log(fernando.getFullName());
  console.log(kelly.getFullName());

  /*
  Mostre no console as idades de cada pessoa, com a frase:
  - "[NOME COMPLETO] tem [IDADE] anos."
  */
  console.log( '\nIdade das pessoas:' );
  console.log(felipe.getFullName() + ' tem ' + felipe.getAge() + ' anos.');
  console.log(fernando.getFullName() + ' tem ' + fernando.getAge() + ' anos.');
  console.log(fernando.getFullName() + ' tem ' + kelly.getAge() + ' anos.');

  /*
  Adicione alguns anos à cada pessoa, e mostre no console a nova idade de
  cada um. A frase deverá ser no formato:
  - "[NOME COMPLETO] agora tem [NOVA IDADE] anos."
  */
  console.log( '\nNova idade das pessoas:' );
  console.log(felipe.getFullName() + ' agora tem ' + felipe.addAge(2).getAge() + ' anos.');
  console.log(fernando.getFullName() + ' agora tem ' + fernando.addAge(2).getAge() + ' anos.');
  console.log(kelly.getFullName() + ' agora tem ' + kelly.addAge(2).getAge() + ' anos.');

}());
